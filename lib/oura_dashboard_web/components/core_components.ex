defmodule OuraDashboardWeb.CoreComponents do
  @moduledoc """
  ...
  import OuraDashboardWeb.Gettext
  ...
  def simple_form(assigns) do
    ...
    <.form :let={f} for={@for} as={@as} {@rest}>
      <div class="mt-10 space-y-8 bg-gray-100 dark:bg-gray-900">
  ...
  def button(assigns) do
    ...
    <button
      type={@type}
      class={[
        "phx-submit-loading:opacity-75 rounded-lg bg-zinc-900 hover:bg-zinc-700 py-2 px-3 dark:bg-amber-500",
  ...
  def input(%{type: "checkbox"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      <label class="flex items-center gap-4 text-sm leading-6 text-zinc-600 dark:text-amber-300">
        <input type="hidden" name={@name} value="false" />
        <input type="checkbox" id={@id} name={@name} value="true" checked={@checked} class="rounded bg-gray-100 dark:bg-gray-800 border-zinc-300 text-zinc-900 dark:text-amber-500 focus:ring-0" {@rest} />
        <%= @label %>
      </label>
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <select id={@id} name={@name} class="mt-2 block w-full rounded-md border border-gray-300 bg-gray-100 dark:bg-gray-900 dark:text-gray-200 shadow-sm focus:border-zinc-400 focus:ring-0 sm:text-sm" multiple={@multiple} {@rest}>
  ...
  def input(%{type: "textarea"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <textarea
        ...
        name={@name}
        class={[
          "mt-2 block w-full rounded-lg text-zinc-900 bg-gray-100 dark:bg-gray-900 dark:text-gray-200 focus:ring-0 sm:text-sm sm:leading-6",
  ...
  def label(assigns) do
    ~H"""
    <label for={@for} class="block text-sm font-semibold leading-6 text-zinc-800 dark:text-amber-500">
      <%= render_slot(@inner_block) %>
    </label>
    """
  end
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between gap-6", @class]}>
      <div>
        <h1 class="text-lg font-semibold leading-8 text-zinc-800 dark:text-amber-500">
          <%= render_slot(@inner_block) %>
        </h1>
        <p :if={@subtitle != []} class="mt-2 text-sm leading-6 text-zinc-600 dark:text-amber-300">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(OuraDashboardWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(OuraDashboardWeb.Gettext, "errors", msg, opts)
    end
  end
