defmodule OuraDashboard.OuraDataStore do
  ...
  @client_zin Tesla.client([
                {Tesla.Middleware.BaseUrl, "https://api.ouraring.com"},
                {Tesla.Middleware.Headers, [{"Authorization", "Bearer #{Application.compile_env(:oura_dashboard, :zin_personal_token)}"}]}
              ])

  @client_balen Tesla.client([
                  {Tesla.Middleware.BaseUrl, "https://api.ouraring.com"},
                  {Tesla.Middleware.Headers, [{"Authorization", "Bearer #{Application.compile_env(:oura_dashboard, :balen_personal_token)}"}]}
                ])
